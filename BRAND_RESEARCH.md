...
**`Mat och Dryck` takes a qualifier after a comma**, and the live data has drifted well past the bare string above: `Mat och Dryck, Bryggeri` (Spendrups), `Mat och Dryck, Bröd` (Polarbröd), `Mat och Dryck, Sprit` (Absolut), `Mat och Dryck, Öl` (Pripps), `Mat och Dryck, Färdigmat` (Felix), `Mat och Dryck, Växtbaserat` (Oatly), **`Mat och Dryck, Mejeri`**, added 2026-08-04 with Arla, Skånemejerier, Norrmejerier and Falköpings Mejeri, **`Mat och Dryck, Juice`**, added 2026-08-05 with Brämhults, and **`Mat och Dryck, Sjömat`**, added 2026-08-05 with Abba. Reuse `Mejeri`, `Juice` and `Sjömat` verbatim for any further dairy, juice or seafood brand (God Morgon and Bravo are the obvious next `Juice` candidates, and both belong to the same group, so they must **reuse `koncern-eckes-granini-international`**). Do not read the one-line list above as exhaustive; query `array::unique(*[_type=="brand"].kategori)` before inventing a string. The deferred `kategori` normalization pass in §9 owns reconciling all of this.

**The qualifier pattern is not confined to `Mat och Dryck`** — the live data applies it across most bases (`Textil, Linne`, `Knivar, Yxor`, `Verktyg, Handsågar`, `Möbler, Inredning, Bygg`). Four more were added with the batch of ten on 2026-08-05 (§10):

| New string | Added with | Reuse it for |
|---|---|---|
| `Verktyg, Yxor` | Gränsfors Bruk | Wetterlings. Chosen over the existing `Knivar, Yxor` (Karesuandokniven) because Gränsfors makes axes and wrecking bars, no knives. |
| `Möbler, Kontorsmöbler` | Kinnarps | Källemo, Lammhults, Offecct, Abstracta, EFG, Materia, Skandiform. Deliberately narrower than the bare `Möbler` used by Hästens, Stolab, Gärsnäs and Swedese, because Sweden has a whole cluster of office-furniture makers still to add. |
| `Mat och Dryck, Sjömat` | Abba | Kalles Kaviar, Grebbestads, Klädesholmen. Preferred over `Fisk` because it also covers kaviar, musslor and tonfisk, and Orkla itself calls the Kungshamn plant its `sjömatsanläggning`. |
| `Friluftsutrustning, Tält` | Hilleberg | Tentipi, and any other tent maker. |

> **Spelling defect for the normalization pass:** the live data holds **`Friluftutrustning, Campingkök`** (Trangia) with the `s` missing from `Friluftsutrustning`. The correct spelling already has precedent in `Skor, Friluftsutrustning`, so `Friluftsutrustning, Tält` was written correctly rather than matched to the typo. **Do not propagate a misspelling for consistency's sake** — two wrong strings are worse than one, and the pass can fix Trangia in isolation.

> **Prefer a qualifier over a bare base when a cluster of siblings is coming.** A one-off brand can take the bare string; a category about to receive half a dozen siblings should get the qualifier on the *first* brand, so the normalization pass has less to undo. `Textil, Linne` already existed from Ekelund Linneväveri and was reused verbatim for Klässbols rather than inventing a `Hemtextil` — **query first, the string you need often already exists.**
...
> **The letter sweep is COMPLETE as of 2026-08-03: all 131 published brands carry `senastVerifierad` and exactly 3 `kallor`.** Groups ran `&` and `A` (2026-07-24), `B`, `C`, `D`+`E`, `F`+`G` (2026-07-25), `H`, `I`+`J` (2026-07-26), `K` (2026-07-27), `L`+`M`, `N` (2026-07-28), `O`, `P`, `R`, all of `S` and `T` (2026-07-29), `V`, `W` and `Z` (2026-08-03). The progress query above now returns empty — **an unverified brand from here on is a newly added brand, not sweep backlog**, so run the §7 checklist on it rather than restarting a letter group. Remaining work is the deferred full-database passes below.

> **The `fel_kontinent` defect class is empty database-wide as of `W`.** Weekday was the last of the ten, cleared 2026-08-03. `*[_type=="brand" && ("Asien" in tillverkningslander || "Europa" in tillverkningslander)]` now returns nothing — keep running it, but treat any new hit as a regression introduced after that date rather than sweep backlog.
...
- **An announced factory CLOSURE is not a closure. The cancelled-factory rule runs in this direction too.** Santa Maria was stored as `Nej` + `["Estland"]` because Paulig had announced the end of Mölndal, but the closure was announced **2026-01-27 and completes "senast sommaren 2027"** — Paulig's own locations page still says, present tense, *"Our Santa Maria spices are produced in Estonia (Saue) and Sweden (Mölndal)"*. The entry became `Delvis` + Sweden restored. **Record what the factory is doing today, not what the press release says it will do**, and put the announced date in the `intro` (same treatment as Polestar's Slovakia and Oatly's Peterborough, mirrored).
- **A brand the database calls "imported" can be the importer's OWN house brand — check the distributor's portfolio page.** `Satake` was stored as a Japanese knife brand with a Japanese koncern, `borsnoterat: "Nej"` and the database's only `brandLand: "JP"`. It is **Vikingsun AB's own label**, created in 1994 for the Nordic market and produced to order by Satake Cutlery in Seki — and Storskogen (Nasdaq Stockholm) bought 95 % of Vikingsun in December 2021. Storskogen's own portfolio page says it outright: *"under the own brands Satake and Culimat"*. One page flipped `borsnoterat`, `brandLand`, the whole koncern and the country list. **When a brand entry reads "svenskdistribuerat", find the distributor and read its own brand list before trusting the foreign-ownership story.**
- **An owner's portfolio page reports PORTFOLIO WEIGHT, not shareholding.** industrivarden.se shows *"33 % Sandvik"*, and a search summary independently offered *"Sandvik (24%)"* — both are Sandvik's share of **Industrivärden's portfolio value**, not Industrivärden's stake in Sandvik, which is **14,9 % of the capital** per Sandvik's own annual report. Two different numbers for the same relationship on the same day is the tell. **Take a shareholding percentage from the issuer's own shareholder table, not from the owner's portfolio breakdown.**
- **A group locations page can name your brand for one product category and stay silent for the others.** Paulig's page attributes **Santa Maria spices** to Saue and Mölndal but lists its tortilla plants (Landskrona, Roeselare, Milton Keynes, Berga) by site with **no brand column**. Brand-specific press releases closed two of those: Landskrona bakes *"400 million Santa Maria tortillas per year"* and Roeselare produces tortillas *"for the Santa Maria and Poco Loco brands"*. Milton Keynes and Spain stayed out for lack of a brand link. Same shape as the H&M group-list problem, but solvable per site.
- **Verify the TOWN, not only the year — and beware a town that belongs to a different company.** Swedese was stored as *"grundad 1945 i Lammhult"*; Swedese's own history page says it still sits *"i samma vackra byggnad i Vaggeryd"* where it started. **Lammhult is Lammhults Möbel AB's town**, a separate furniture company, which is almost certainly where the error came from. Two Småland furniture makers, one famous furniture town, one wrong field. **Thule repeated the pattern one group later**: stored as *"grundat 1942 i Malmö"*, but Malmö is the head office and Thule was founded in **Hillerstorp** by Erik Thulin. Both errors substitute the *current HQ* for the founding place, so check the founding town against the company's own history page whenever the stored town is also its HQ city.
- **A brand's operating company and its trademark owner can be two unrelated groups.** Tretorn's `agare` said "Authentic Brands Group" (US), which is true of the **trademark** — Puma sold ABG the rights in 2015. But hitta.se shows the operating company **Tretorn Sweden AB** (48 employees, Helsingborg, with Tretorn subsidiaries in Benelux, Finland and Norway) sitting under **Thevea Brands Group AB → Thevea Holding AB**, a Swedish group. This is the IKEA Ingka-vs-Inter split in licence form: a US brand-management firm owns the name while a local group runs the business and commissions production. Record the side that commissions production, and name the trademark owner in the `intro`.
- **A `/policies/terms-of-service` page identifies the company that took over after a BANKRUPTCY, not just the original founder's entity.** TRIWA AB went bankrupt in February 2024 and press reported a group led by David Wendel buying the estate. TRIWA's own terms page names **Golden Triangel AB** (559302-0307) as the operator, whose hitta.se page gives **Gyllene Triangeln AB** as parent — a watch-trading group. That turned `koncern: null` into a real two-level chain. **The Myrqvist rule extends to successions:** after any bankruptcy or restructuring, read the current terms page before trusting press coverage of who bought what.
- **A supplier list's country column can be the supplier's REGISTERED ADDRESS, not the factory.** Tiger of Sweden's own tier 1 list (updated 2025-06-03) names ten countries, one of them **Hong Kong** — but every Hong Kong row is an office address in a commercial tower (*"Room 2602, 26/F, Orient International Tower"*, *"5/F, TAL Building"*), and TAL Global Alliances is a group whose plants are elsewhere in Asia. Hong Kong was excluded and the other nine kept. This is the Haglöfs rule applied to a supplier list rather than a single supplier: when the list has no role column, read the **addresses**, and treat a 26th-floor office as sourcing, not sewing.
- **Prefer specific on-site reporting over an absolute claim, and beware a domestic "production line" that produces nothing.** Multiple secondary sources assert *"All TE products are made in Sweden, without exception"* for Teenage Engineering. iFixit's factory piece (2024-01-22) reports the opposite in detail: assembly happens at TE's **Barcelona** plant, chosen from 14 European candidates, and *"Teenage Engineering actually has two versions of its production line. One in the Barcelona factory, and one in its office in Stockholm. The Stockholm one doesn't manufacture anything"* — it exists to design and test the process. The stored `Nej` survived because of that sentence. An absolute claim ("without exception") is a smell; look for someone who has actually walked the floor.
- **A delisting years in the past can hide behind an enum break.** Tiger of Sweden carried `borsnoterat: "Ja (del av IC Group)"`. The enum violation was the visible bug, but the substance was wrong too: IC Group's **last trading day was 2 August 2019** after Friheden Invest took it private, stated on IC Group's own site. Fixing the enum without re-checking the ownership chain would have produced a valid-looking `"Ja"` that is still false. **Whenever you clear an annotated enum, re-verify the claim the annotation was making.**
- **Re-check `borsnoterat` on the household names, not only the obscure ones.** Volvo Cars sat in the database as `"Nej"` while Volvo Car AB (publ) has traded on **Nasdaq Stockholm as VOLCAR B since October 2021** — a one-search check that no one had run, because a famous entry looks like it must already be right. The same brand's koncern also pointed one level too low, at the operating subsidiary **Volvo Car Corporation** instead of the listed parent **Volvo Car AB**. For any large group, name the *listed* entity in `moderbolag`.
- **A company's own press-release boilerplate can be years out of date; prefer the standing fact page.** A 2020 Volvo Cars release still said *"under the ownership of the Zhejiang Geely Holding since 2010"* and listed plants without Taizhou; the maintained **"This is Volvo Cars"** media fact page carries the current plant list plus the listing and ticker. First-party is not the same as current, and a dated release ages while a fact page is edited.
- **A group site that puts a BRAND COLUMN on each plant solves the Paulig/H&M problem outright.** `volvogroup.com/en/about-us/organization/our-production-facilities.html` names the brand(s) built at every site, which took Volvo Trucks from 4 to 8 countries (adding France, India, South Africa and Australia) with no inference at all. Look for a production-facilities page with per-plant attribution before falling back to brand-specific press releases.
- **northdata can invert a parent/subsidiary direction — fetch the suspected parent's OWN page.** For Vagabond, northdata reported BYOS Bring Your Own Shoes AB's parent as *Vagabond International AB*; BYOS's own hitta.se page names **Familjen Nilsson Peterzéns Stiftelse** as its `Moderbolag`, and the operating company sits under BYOS. The registration date settled it: BYOS was registered 2023-10-03, two months before the founders gifted the company to the foundation on 2023-12-31. When two registries disagree on direction, fetch the page *of the entity you think is on top* and check the deal timeline.
- **For shareholders plus the listing sentence, fetch the Corporate Governance Report, not the annual report.** Volvo Group's 8 MB Annual Report 2025 came back empty through `r.jina.ai` (the Polestar 20-F failure mode), while `corporate-governance-report-2024.pdf` rendered fully and states both *"AB Industrivärden 28.0 percent of the votes"* / *"Geely Holding 14.7 percent"* and the Nasdaq Stockholm admission. Volvo Cars' CG report did the same job for Geely Sweden Holdings' 78.65 %. The CG report is the small PDF that holds exactly the two facts this sweep needs.
- **`volvogroup.com` times out on WebFetch and `volvocars.com` returns 403 — both render through `r.jina.ai`.** Two more hosts for the reachability table: the timeout is not the page being dead, and the 403 is bot-blocking, not a broken URL. Cite the human URL, fetch it through the proxy.
- **An election committee's membership is a live ownership signal.** AB Volvo's stored `agare` blob led with *"Geely 19.88%"*; the 2026-02-19 board proposal shows the committee representing **Industrivärden, AMF, Alecta and AFA (~39.8 % of votes)** with **no Geely seat**, which is what prompted checking the actual figures. A named-shareholder blob that no longer matches who sits on the nomination committee is stale by definition.
- **A stored `moderbolag` can be a MATERIAL name, not a company at all.** Woolpower's parent read `"Ulfrotté AB"` — Ullfrotté Original is the *fabric* Woolpower knits, misspelled and given an AB suffix. No such company exists; the real chain is `Ekstigen Förvaltning AB` → `Ekstigen AB` (hitta.se, one fetch). This is a fourth outcome alongside the three self-referential cases: the stored name is not a former name, not a fabricated *company* name, but a **product or material name promoted to an entity**. Check that the string is even a plausible company before hunting for the registry entry, and note the tell: the brand's own about page named the material and the owner family in the same paragraph.
- **A SPA press release can be unfetchable even through `r.jina.ai` — then cite the trade outlet that quotes it verbatim.** `wastberg.com/en/newsroom/press-releases/...` returns the site's *support* page through both WebFetch and the proxy (client-side routing, no server-rendered article). The sentence that decided Wästberg's country list, *"produce the majority of their lamps in XAL's state of the art production facilities in Austria and Slovenia"*, exists in full on `led-professional.com`. A trade article quoting a first-party release is a legitimate `kallor` URL when the original cannot be retrieved — say so in the report, and prefer the outlet that quotes rather than paraphrases.
- **Operational control is not shareholding, and the koncern field follows shareholding.** XAL Group lists Wastberg+ as one of its three brands and has installed its own CEO and *entire* board at the Swedish company, yet the only documented shareholding still has the founder as majority owner and XAL as sole minority partner, and hitta.se shows no `Koncernmoderbolag`. `koncern` stayed `null` (the Chimi precedent) with XAL named in the `intro`. **A group that markets a brand as its own is making a branding claim, not filing a share transfer** — record the control signals as a watch item instead of inventing a parent.
...
- [ ] **Placeholder `agare` — the final work list is four documents.** With the sweep finished, `koncern_platshallare` is down from 17 to **four live cases**: `koncern-a-day-s-march-ab`, `koncern-acne-studios-ab` and `koncern-all-blues-ab` (all from `A`, verified before these criteria existed, all with a self-referential `moderbolag` as well) plus **`koncern-stutterheim-ab`**, whose `moderbolag` "Aktiebolaget Svenska Regn" is genuine and correct while `agare` was never filled in. The other five hits are **orphaned** koncerns with 0 brands (Byarums Bruk, Jeansverket, Kasthall, Kero Försäljning, Steel by Lundbergs) — leave them. `& Other Stories` and `ARKET`, the other early-group casualties, were retrofitted at `I`+`J`.
- [ ] **Two `Delvis` brands name only Sweden.** `Sätila of Sweden` and `Skultuna Messingsbruk` are `Delvis` with `["Sverige"]`, i.e. the Kosta Boda honest gap applied to the *status* field rather than the country list: production abroad is asserted (Sätila's sewing partners "in northern Europe") but no country can be named. They are **expected** `delvis_ett_land` hits alongside the three expected `tom_landlista` ones, not regressions. Revisit only if either company ever names a country.
- [x] **The audit's one known data defect is closed** (found 2026-07-29, cleared at `V` on 2026-08-03): **Verso Skincare**'s `tillverkningslander: null` is now an explicit `[]`. Its own FAQ says only *"Verso produces its products within the European Union"* and no retailer PIM or label route names a country, so this is the Kosta Boda gap, not a lookup failure. **The `null` entry in `array::unique(*[_type=="brand"].tillverkningslander[])` disappears with it.** (Skultuna Messingsbruk's `"Globalt"` was cleared at `S`; Tiger of Sweden's `borsnoterat: "Ja (del av IC Group)"` enum break at `T`, where it also turned out to be substantively wrong.)
- [ ] **Five brands hold a deliberately EMPTY `tillverkningslander`** — Tretorn and TRIWA (Maximilian's call at `T`), Verso Skincare (`V`), and **Axis Communications and Byredo**, both added 2026-08-05 with the batch of ten (§10). This is the endpoint of the Kosta Boda rule: the value had to go, no source names a country, and the `intro` carries the gap instead. They are expected `tom_landlista` hits, not regressions. Revisit if any of the five ever publishes an origin. To find genuine null *elements* (as opposed to an empty array) use `count(tillverkningslander[!defined(@)]) > 0`.

  > **The empty-list outcome is now a recognisable brand *type*, not a one-off.** All five are Swedish-founded, design-led companies with no owned factory that decline to name a production country: two fashion/accessory brands, one skincare, one fragrance, one electronics. **Axis** is the most striking, because a company with 68-plus production facilities' worth of output says only that it uses external EMS partners, and its TAA-compliance page (which exists precisely to assert country of origin for US government buyers) still names no country. **Byredo** survived four independent routes: its own about page, its Puig owner pages, Swedish retailer PIM at Kicks and Lyko, and a product page fetched for regulatory text. When you hit this pattern, do not keep digging past the point of diminishing returns and do not accept blog-grade claims: SEO content will happily tell you Axis builds in Poland, Mexico and Thailand, which is the same evidence class §9 rejected for Kosta Boda's Turkiet/Slovakien. Ship `Nej` + `[]` and state the gap in the `intro`.
...
## 10. Field findings from the 2026-08-05 batch of ten

Ten brands were researched and published on 2026-08-05, chosen to span the decision space rather than by fame: **Gränsfors Bruk** (Ja), **Klässbols Linneväveri** (Ja), **Pågen** (Ja), **Kinnarps** (Ja) and **Abba** (Delvis) in the first wave, 136 → 141 brands; **Hilleberg** (Nej), **Kavat** (Delvis), **Byredo** (Nej), **Axis Communications** (Nej) and **Atlas Copco** (Delvis) in the second, 141 → 146. All ten carry three `kallor` and `senastVerifierad: 2026-08-05`, and every §9 pass-criteria and integrity query returned 0.

> **All five `brand-researcher` dispatches in wave 2 died on API 529 Overloaded, twice, while the main session's own tool calls kept working.** Nothing had been staged, so unlike the `L`+`M` cap-deaths there was no transcript recovery to do. **The fix was to research all five in the main loop directly**, which is the same fallback the session before `R` used. If subagents fail repeatedly while your own WebFetch calls succeed, stop relaunching them and do the work inline; publish per brand as each one finishes so a later failure cannot cost you the batch.

**Ownership and koncern:**

- **Check whether a group koncern already exists under an unrelated-looking `_id` before creating one.** Gränsfors Bruk needed no new koncern: the Ekstigen group document already existed as **`koncern-ulfrott-ab`**, created for Woolpower at `W`. Nothing in that id suggests it covers axes. Query by `moderbolag`/`agare` **values**, never by guessing the id. Wetterlings, Svedbro Smide and Bröderna Smedbergs all reuse it too.
- **A shared koncern does NOT imply a shared `tillverkadISverige`.** Abba (`Delvis`) and Felix (`Ja`) now sit in the same Orkla koncern legitimately, because they are different products from different plants. This is *not* the Kosta Boda/Orrefors situation, where the contradiction mattered because both brands drew on one glassworks network. Check whether the siblings share a *factory* before assuming a sibling's value must change.
- **northdata's free tier inverted the parent direction twice in one batch** (Kinnarps: reported the subsidiary as parent of its own holding; Pågen: reported the subsidiary Lilla Harrie Valskvarn, then Future Y AB, as Pågengruppen's parent). Both were caught by hitta.se disagreeing. Treat northdata as authoritative for the **absence of a listing** and for company form, and as unreliable for **direction of ownership**.
- **`hitta.se`'s ABSENT `Moderbolag` field, positively confirmed, is what makes an independent entry defensible.** Hilleberg the Tentmaker AB has no `Moderbolag` and is itself koncernmoderbolag over three subsidiaries, so it gets **no koncern reference** at all (the Klättermusen and Morjas precedent). Say "the field is absent" explicitly in the report; it is a finding, not a gap.
- **For a listed Swedish group, `moderbolag` = the listed AB itself and is NOT the self-referential defect.** Atlas Copco AB, like the existing Electrolux AB, Sandvik AB, Husqvarna AB and Telefonaktiebolaget LM Ericsson, sits at the top of its own chain, with `agare` carrying `"Börsnoterat (<largest owner> <n> % av rösterna)"`. Get the percentage from the company's own shareholder page, not a registry.
- **`Klässbols Handväveri AB` is a genuine parent that LOOKS like the §5 self-referential defect** (same surname, same address, near-identical name) but has a distinct corporate purpose. Do not "fix" it in a later sweep.
- **`agareLand: "LI"` was already precedented** by IKEA's `Inter IKEA Foundation`, so Pågen's Liechtenstein family office needed no schema thought. Dump `array::unique(*[_type=="koncern"].agareLand)` before assuming a country code is new.

**Manufacturing and the verdict:**

- **A brand's own factory list is the single most valuable manufacturing source, when it exists.** `kavat.com/se/fabriker` names twelve factories, their towns and countries. Look for `/fabriker`, `/factories`, `/vara-leverantorer` or a supplier PDF before anything else. Note `kavat.com` returns **403** to direct fetching but works through the `r.jina.ai` proxy.
- **A closed home factory does not settle the verdict, because the supplier list can put Sweden back.** Kavat shut its Kumla factory in **2013** and sold its Bosnian factory in **2023**, which reads as an obvious `Nej`. But one of the twelve factories it publishes is **Brasko / AB Mathisens Skofabrik in Åbytorp**, 2 km from Kumla, a genuine active Swedish shoe factory making 50-60 000 pairs a year. That makes Kavat **`Delvis`**. This is the Nudie/Sätila rule reached from the opposite direction: there, a Swedish supplier row upgraded a brand nobody expected; here it rescued a `Ja`-flavoured history from being written off. Brasko is itself worth adding later.
- **A company-owned foreign factory is still foreign production.** Hilleberg is Swedish, family-owned, and owns its Rapla factory outright, yet has sewn nothing in Sweden since moving production out in **1996/97**, so it is `Nej`. Many people would assume `Ja`. **A second country turned up that no plan anticipated:** since 2023 the Blue Label tents are made in **Hanoi, Vietnam**, at a plant owned by pole supplier DAC. Read the whole sustainability page rather than stopping at the first factory named.
- **Kinnarps went `Ja` on closed foreign plants, which is a fact you have to go looking for.** Minden (2014) and Worms (2018) both shut with production moved to Jönköping and Skillingaryd. A brand with a historical foreign plant reads as `Delvis` unless you confirm the closure.
- **Pågen's Landskrona bakery is the Oatly rule in the positive direction:** real, under construction, first stage end of 2029, and correctly kept **out** of `tillverkningslander` while going into the `intro` as planned capacity.
- **For a global industrial, the annual report's per-business-area "manufacturing units are located in" sentence is the country list.** Atlas Copco's 2025 report gives one such sentence per business area, and **Sweden appears in only one of four** (Industrial Technique), which is exactly what justifies `Delvis` over `Ja`. The union of the four is ten countries, comparable to Scania's eleven. `r.jina.ai` extracted it from the ESEF PDF where `WebFetch` could not; per §1 it was **run twice and the overlapping quote matched** before being trusted. The first run silently found only one of the four areas, so **ask for every occurrence of the exact phrase rather than asking per business area.**
- **Do not attribute a spun-off division's plants to the parent.** Atlas Copco's Örebro rock-drill operation is **Epiroc**, separately listed since 2018. Search results still conflate them.
