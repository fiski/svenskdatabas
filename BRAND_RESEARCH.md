...
- [ ] **Three brands hold a deliberately EMPTY `tillverkningslander`** — Tretorn and TRIWA (Maximilian's call at `T`) and Verso Skincare (`V`). This is the endpoint of the Kosta Boda rule: the value had to go, no source names a country, and the `intro` carries the gap instead. They are expected `tom_landlista` hits, not regressions. Revisit if any of the three ever publishes an origin. To find genuine null *elements* (as opposed to an empty array) use `count(tillverkningslander[!defined(@)]) > 0`.
